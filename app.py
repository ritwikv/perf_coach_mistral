...
# Function to evaluate CSR answers using Deepeval
def evaluate_answers(df: pd.DataFrame, rag_answers: List[str]) -> List[Dict[str, Any]]:
    ...
    for i, row in df.iterrows():
        ...
        def custom_llm_evaluator(prompt):
            response = model(prompt, max_tokens=500, temperature=0.1)
            return response['choices'][0]['text'].strip()
